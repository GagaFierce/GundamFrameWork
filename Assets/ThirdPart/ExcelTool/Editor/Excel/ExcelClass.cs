    ...
    class ExcelClass
    {
        ...
        public ExcelClass(string _filename, string _savepath)
        {
            ...
            try
            {
                if (fileName.EndsWith(".xls") || fileName.EndsWith(".xlsx") || fileName.EndsWith(".xlsm")) 
                    ...
                    Debug.LogError("只支持xls 和 xlsx格式。");
            }
            catch (Exception ex)
            {
                Debug.LogError("_filename:"+_filename);
        ...
        public void SaveToJson()
        {
            ...
            foreach (var curSheet in mWorkbook.Worksheets)
            {
                ...
                if(!curSheet.IsVisible) continue;
                
                ExcelData tdata = GetContentHaveValue(curSheet);
                string tfullname = savepath + "/" + "Conf"+curSheet.Name + ".json";
        ...
        public void SaveFile()
        {
            ...
            foreach (var curSheet in mWorkbook.Worksheets)
            {
                ...
                ExcelData tdata = GetContentHaveValue(curSheet);
                string tfullname = savepath + "/" + "Conf"+curSheet.Name + ".bytes";
        ...
        public Dictionary<string, List<string>> ExportReadClass()
        {
            ...
            foreach (var curSheet in mWorkbook.Worksheets)
            {
                ...
                ExcelData tdata = GetContentHaveValue(curSheet);
                string tfullname = savepath + "/" + "Conf"+curSheet.Name + ".cs";

                ExportToCS tcs = new ExportToCS("Conf"+curSheet.Name, tfullname, tdata);
                bool isSuccess = tcs.StartExport();

                if (isSuccess)
                {
                    ret[curSheet.Name] = tcs.GetFieldList();
                }
