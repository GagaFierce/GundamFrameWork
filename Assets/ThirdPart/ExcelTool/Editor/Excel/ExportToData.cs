...
namespace ExportTool
{
    public class ExportToData
    {
        ...
        public void StartExport()
        {
            try
            {
                ...
                for (int i = ExcelData.sStartLine; i < data.r; i++)
                {
                    ...
                    {
                        Debug.LogFormat($"导出截至到第{i}行。filename = {filename}");
                        break;
                    }
                ...
                Debug.Log($"导出结束。共 {twriteLiine} 行数据");
            }
            catch (Exception ex)
            {
                Debug.LogError(ex);
        ...
        void ShowError(int i,int j,string pErro)
        {
            ...
            {
                Debug.LogError($"表 {filename} 生成配置出现错误第{i}行,第{j}列.erro = {pErro}");
            }
        }
        ...
        protected void WriteValue(BinaryWriter _write, string _typestr, string _value)
        {
            ...
            switch (_typestr)
            {
                ...
                case "short":
                    ...
                    break;
                case "bool":
                    if (int.TryParse(_value, out var intValue) && intValue == 1)
                    {
                        _write.Write(true);
                    }
                    else if (bool.TryParse(_value, out var boolValue) && boolValue)
                    {
                        _write.Write(true);
                    }
                    else
